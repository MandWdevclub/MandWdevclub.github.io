...
I don't know about you, but that was way too exciting for me, let's slow it down a bit. Let's configure uwsgi to work with our flask application. First off, what is uwsgi? uwsgi is a pre-made server which is capable of interoperating with frameworks that run on wsgi like flask or django. The main idea here is that python has a protocol specification called wsgi, and so to serve our flask application, we need a server that can communicate properly with flask via wsgi. All of these things are open source, so please feel free to contribute to them if and when you feel comfortable.


First, create a file ```wsgi.py``` and add the following code:
...
if __name__ == "__main__":
    app.run()
```
This file will serve as a way to pluck our ```app``` module from our ```myproject.py``` file, and then tell uwsgi that this is the application that will be receiving data.
...
This will run the uwsgi server and configure it to use a socket with port 5000, and to use http protocol instead of wsgi protocol. Finally, the ```-w``` argument supplies the name of the entry point file and it's type, or ```wsgi.py``` with type ```app```.
...
Cool, OK, sorry to get your hopes up, but we still need to configure more uwsgi stuff. Trust me m8, you don't want to have to be entering ```uwsgi --socket 0.0.0.0:5000 --protocol=http -w wsgi:app``` everytime you want to run your server, so let's automate it.
...
Lemme break this jive sucka down for you. ```[uwsgi]``` is just a tag for all uwsgi configuration files. ```module = wsgi:app``` is basically identical to what we were entering on the terminal before: ```module = <insert module file name>:<insert callable object for flask>```. ```socket = myproject.sock``` is another important aspect. We'll be using another server later, nginx, and nginx will be more efficient communicating via a unix socket. We specify the name of this socket for uwsgi to listen to. The rest of the file isn't too important, just setting up startup, termination, and security parameters. Feel free to ask an instructor about these extra parameters.

Now that we've automated uwsgi, let's automate the actual ubuntu instance to run uwsgi on startup. Go ahead and create the following ```systemd``` file:
